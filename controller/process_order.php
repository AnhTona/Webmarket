...
class OrderManager {
    ...
    public function saveOrderItems(int $orderId, array $cart): void {
        ...
        foreach ($cart as $item) {
            ...
            if ($productId) {
                ...
                // ✅ TRỪ SỐ LƯỢNG TỒN KHO NGAY SAU KHI THÊM CHI TIẾT
    ...
    private function decreaseStock(int $productId, int $quantity): void {
        ...
        $stmt = $conn->prepare("SELECT TenSanPham, SoLuongTon FROM sanpham WHERE MaSanPham = ?");
        ...
        if ($product['SoLuongTon'] < $quantity) {
            throw new Exception("Sản phẩm '{$product['TenSanPham']}' chỉ còn {$product['SoLuongTon']} sản phẩm, không đủ để đáp ứng yêu cầu {$quantity}");
        }
        ...
        if ($stmt->affected_rows === 0) {
            throw new Exception("Không thể trừ tồn kho cho sản phẩm '{$product['TenSanPham']}'");
        }
    }
...
class OrderProcessor {
    ...
    public function processOrder(array $postData): array {
        ...
        try {
            ...
            // 5. Lưu chi tiết đơn hàng (✅ TỰ ĐỘNG TRỪ TỒN KHO BÊN TRONG)
